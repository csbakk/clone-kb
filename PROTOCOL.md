...
| **세션 결산** | ①`ledger/`에 사용 기법 판정 append(성과/실패/중립+증거 링크) ②**런 매니페스트 §4 로직 평가 채움**(작동한 것/병목/다음 런에서 바꿀 것 — 세션 '로직 자체'의 평가, ledger·승격의 근거) + status:running→done ③**아침 보고 HTML**(무인 런일 때): 프로젝트 repo `reports/<날짜>-morning.html` 자체완결 1장 — **고정 8섹션**: ①헤더(세션·커밋범위·MD 정본 링크) ②판정 카드 4(게이트·핵심 수치·발견·안전) ③수치 추이 ④시각 증거(트리아지류 자료가 있으면 발췌 임베드+원본 링크 — 별도 요청 불요) ⑤밤 사이 발견 표 ⑥오너 액션 큐(체크박스, 결정 필요한 것만) ⑦다음 밤 큐 ⑧용어 사전(**접기 details**) + 본문 반복 용어 점선 툴팁(CSS data-tip — 네이티브 title 금지, 지연·미발화) — 오너가 모르는 용어 없이 읽히는 게 기준. **결정 회수**: 모든 체크박스·메모는 localStorage 퍼시스트 + 우하단 "📋 결정 복사" 버튼(클립보드 텍스트 → 오너가 클로드 채팅에 붙여넣으면 반영). 정적 HTML 체크는 자동 전달되지 않음을 오너에게 고지. ⚠스크린샷 포함이므로 **private 프로젝트 repo에만**, 이 KB(public)엔 경로 포인터만 ④`cases/<프로젝트>.md` 진행·도식 갱신 ⑤`gen_dashboard.py` 재생성 ⑥push. **역할 분리 원칙: HTML=판독·비교 뷰(레이아웃 제어) / MD=기록·축적 정본(diff·검색·링크)** |
