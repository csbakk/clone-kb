...
| **세션 결산** | ①`ledger/`에 사용 기법 판정 append(성과/실패/중립+증거 링크) ②**런 매니페스트 §4 로직 평가 채움**(작동한 것/병목/다음 런에서 바꿀 것 — 세션 '로직 자체'의 평가, ledger·승격의 근거) + status:running→done ③**아침 보고 HTML**(무인 런일 때): 프로젝트 repo `reports/<날짜>-morning.html` 자체완결 1장 — 게이트 판정·수치 추이·시각 증거(크롭)·오너 액션 큐(체크박스). ⚠스크린샷 포함이므로 **private 프로젝트 repo에만**, 이 KB(public)엔 경로 포인터만 ④`cases/<프로젝트>.md` 진행·도식 갱신 ⑤`gen_dashboard.py` 재생성 ⑥push. **역할 분리 원칙: HTML=판독·비교 뷰(레이아웃 제어) / MD=기록·축적 정본(diff·검색·링크)** |
