...
> **★★기본값이다 — 재주입 불요(2026-07-19, 오너 "내내 말했는데" 지적 후 확정)**: 어떤 요소를 만들거나 고치든(신규·버그 수복·placeholder·상태·빈 화면 전부 예외 없음) "대충 화면 흉내"·근사·합성 데모로 끝내지 말고 **실물에서 최하단 자식부터 골격·구조·스타일·상태·애니메이션을 실측해 1:1 복사**한다. 오케가 매 워커 브리프에 재주입하지 않아도 워커가 리포 `_POLICY.md §0`에서 읽고 무조건 적용한다. 오너가 같은 지시를 반복하게 만드는 것 자체가 시스템 결함 — 이 규율을 정책 최상위에 박아 예방한다.
