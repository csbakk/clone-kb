...
- isolated 23,581→23,338(-243, 1차 아크), 크레딧 0.
- 2·3차 아크(첫 결산 후 연장): P7(-139)·P8(restore_node_floor 하드닝)·P9(프론티어 +100스텝, 진짜갭 LLM패널·통지대기 2회 재발)·P10(-188). isolated 최종 23,581→23,011(-570, 8배치 역행 0).
...
- **병목/실패**: ①델타 큐가 근본적으로 스테일이라 3페이즈(GEN-TOAST-1·필터탭바·assets_panel)를 유령에 소모 — 근본수정 전까지 반복 ②"역행 0" 문구 과대(재덤프 동적표면 노이즈, AD-D1) — "대상상태 역행0"으로 정밀화 필요 ⑤★"통지 대기" 정지 재발(P9 2회, 브리프 명문화에도)—오케 SendMessage 개입 필요, night-run-sop 코드 강제 검토 ⑥탐사 노드플로어 새 실패모드(삭제클릭 네비)로 자동정리 실패 ③탐사 clone 커버리지 7.4%(허브 후보폭발로 정체) ④i18n 보류 4건(실물 덤프 부재).
